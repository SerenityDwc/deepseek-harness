...
describe('isTrustedApiRequest', () => {
  it('holds markerless requests to the same Host fence — a plain-HTTP browser read carries no markers', () => {
    ...
    expect(isTrustedApiRequest(request({ host: '192.168.1.5:3080' }), [], { ipLiterals: true })).toBe(true)
    expect(isTrustedApiRequest(request({ host: '[2001:db8::1]:3080' }), [], { ipLiterals: true })).toBe(true)
    expect(isTrustedApiRequest(request({ host: 'harness.example' }), [], { ipLiterals: true })).toBe(false)
  ...
  it('refuses malformed or untrusted authorities on browser requests', () => {
    ...
    expect(isTrustedApiRequest(request({ ...markers, host: '128.0.0.1' }), [], { ipLiterals: true })).toBe(true)
  })
})
