...
export class SettingsScopeController<T> implements SettingsScope<T> {
   ...
   * @param persistence - DNS-named remote browsers remain process-local because settings RPCs stay loopback-only there.
...
export class SettingsScopeBinder extends Service {
  ...
  bind<T>(spec: SettingsScopeSpec<T>): SettingsScope<T> {
    ...
    const controller = new SettingsScopeController<T>(
      ...
      spec,
      this.mirror,
      connection.settingsOnHost ?? connection.isLoopback ? 'host' : 'memory',
      this.schema,
    )
    ctx.effect(() => {
