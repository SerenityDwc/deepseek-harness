...
- Non-loopback deployments must have their serving authorities trusted or requests are refused. The dsh CLI rejects `dsh web --host 0.0.0.0`; an all-interfaces bind comes from a composition overlay such as the [Docker Web image](../feature/2026-08-24-docker-web-image.md). The Web runtime still derives the machine's LAN IP literals into the connection row (port-less entries). Ordinary `/api` routes also accept a canonical IP-literal Host without listing it, because an IP-literal Host cannot be a rebound DNS name; privileged methods do not receive that grant unless `privilegedIpLiterals` is true. `dsh web --trusted-host` and cordis.yml `trustedHosts` declare named authorities; compositions the CLI does not boot declare `trustedHosts` themselves. Non-browser automation rides the same fence: loopback, an IP literal, a derived LAN IP, or a declared authority passes; an undeclared DNS alias is refused.
